...
class q {
  ...
  static async loadFromPath(A) {
    ...
    if (typeof Bun < "u" && typeof Bun.file == "function")
      try {
        const E = Bun.file(A);
        await E.exists() && (Q = await E.arrayBuffer());
      } catch {
      }
    if (!Q)
      try {
    ...
    if (!Q) {
      ...
      if (!E.ok)
        throw new Error(`Failed to fetch WASM: ${E.status} ${E.statusText}`);
      if (Q = await E.arrayBuffer(), Q.byteLength === 0)
        throw new Error(`WASM file is empty (0 bytes). Check path: ${A}`);
    }
    if (!Q)
      throw new Error(`Could not load WASM from path: ${A}`);
    const g = await WebAssembly.compile(Q), C = await WebAssembly.instantiate(g, {
      env: {
        log: (E, I) => {
          const D = new Uint8Array(
            C.exports.memory.buffer,
            E,
            I
          );
...
class W {
  constructor(A) {
    ...
    const C = new DataView(this.exports.memory.buffer);
    this.encoder = C.getUint32(Q, !0), this.exports.ghostty_wasm_free_opaque(Q);
  }
  setOption(A, Q) {
  ...
  setKittyFlags(A) {
    this.setOption(p.KITTY_KEYBOARD_FLAGS, A);
  }
  encode(A) {
    ...
    const C = new DataView(this.exports.memory.buffer), E = C.getUint32(Q, !0);
    if (this.exports.ghostty_wasm_free_opaque(Q), this.exports.ghostty_key_event_set_action(E, A.action), this.exports.ghostty_key_event_set_key(E, A.key), this.exports.ghostty_key_event_set_mods(E, A.mods), A.utf8) {
      const M = new TextEncoder().encode(A.utf8), a = this.exports.ghostty_wasm_alloc_u8_array(M.length);
      new Uint8Array(this.exports.memory.buffer).set(M, a), this.exports.ghostty_key_event_set_utf8(E, a, M.length), this.exports.ghostty_wasm_free_u8_array(a, M.length);
    }
    ...
    if (w !== 0)
      throw this.exports.ghostty_wasm_free_u8_array(D, I), this.exports.ghostty_wasm_free_usize(o), this.exports.ghostty_key_event_free(E), new Error(`Failed to encode key: ${w}`);
    const s = C.getUint32(o, !0), t = new Uint8Array(this.exports.memory.buffer, D, s).slice();
    return this.exports.ghostty_wasm_free_u8_array(D, I), this.exports.ghostty_wasm_free_usize(o), this.exports.ghostty_key_event_free(E), t;
  }
  dispose() {
...
const z = class y {
  constructor(A, Q, g = 80, C = 24, E) {
    var I;
    if (this.viewportBufferPtr = 0, this.viewportBufferSize = 0, this.cellPool = [], this.graphemeBuffer = null, this.graphemeBufferPtr = 0, this.exports = A, this.memory = Q, this._cols = g, this._rows = C, E) {
      const D = this.exports.ghostty_wasm_alloc_u8_array(f);
      if (D === 0)
        throw new Error("Failed to allocate config (out of memory)");
      try {
        const o = new DataView(this.memory.buffer);
        let w = D;
        o.setUint32(w, E.scrollbackLimit ?? 1e4, !0), w += 4, o.setUint32(w, E.fgColor ?? 0, !0), w += 4, o.setUint32(w, E.bgColor ?? 0, !0), w += 4, o.setUint32(w, E.cursorColor ?? 0, !0), w += 4;
        for (let s = 0; s < 16; s++)
          o.setUint32(w, ((I = E.palette) == null ? void 0 : I[s]) ?? 0, !0), w += 4;
        this.handle = this.exports.ghostty_terminal_new_with_config(g, C, D);
      } finally {
        this.exports.ghostty_wasm_free_u8_array(D, f);
      }
    } else
      this.handle = this.exports.ghostty_terminal_new(g, C);
    if (!this.handle)
  ...
  // ==========================================================================
  write(A) {
    const Q = typeof A == "string" ? new TextEncoder().encode(A) : A, g = this.exports.ghostty_wasm_alloc_u8_array(Q.length);
    new Uint8Array(this.memory.buffer).set(Q, g), this.exports.ghostty_terminal_write(this.handle, g, Q.length), this.exports.ghostty_wasm_free_u8_array(g, Q.length);
  }
  resize(A, Q) {
   ...
   */
  getViewport() {
    const A = this._cols * this._rows, Q = A * y.CELL_SIZE;
  ...
  getLine(A) {
    ...
    return Q.slice(g, g + this._cols).map((C) => ({ ...C }));
  }
   ...
   */
  getScrollbackLine(A) {
    const Q = this._cols * y.CELL_SIZE;
    ...
    if (g < 0)
      return null;
    const C = [], E = this.memory.buffer, I = new Uint8Array(E, this.viewportBufferPtr, g * y.CELL_SIZE), D = new DataView(E, this.viewportBufferPtr, g * y.CELL_SIZE);
    for (let o = 0; o < g; o++) {
      const w = o * y.CELL_SIZE;
      C.push({
  ...
  readResponse() {
    ...
    try {
      ...
      return new TextDecoder().decode(C.slice());
    } finally {
  ...
  parseCellsIntoPool(A, Q) {
    const g = this.memory.buffer, C = new Uint8Array(g, A, Q * y.CELL_SIZE), E = new DataView(g, A, Q * y.CELL_SIZE);
    for (let I = 0; I < Q; I++) {
      const D = I * y.CELL_SIZE, o = this.cellPool[I];
      o.codepoint = E.getUint32(D, !0), o.fg_r = C[D + 4], o.fg_g = C[D + 5], o.fg_b = C[D + 6], o.bg_r = C[D + 7], o.bg_g = C[D + 8], o.bg_b = C[D + 9], o.flags = C[D + 10], o.width = C[D + 11], o.hyperlink_id = E.getUint16(D + 12, !0), o.grapheme_len = C[D + 14];
    }
  }
  /**
  ...
  getGrapheme(A, Q) {
    ...
    if (g < 0)
      return null;
    const C = new Uint32Array(this.memory.buffer, this.graphemeBufferPtr, g);
    return Array.from(C);
  }
  /**
...
class m {
  ...
  getLine(A) {
    ...
    if (this.bufferType === "normal" && A < g) {
      const D = A;
      C = Q.getScrollbackLine(D), I = !1;
    } else
      E = this.bufferType === "normal" ? A - g : A, C = Q.getLine(E), I = Q.isRowWrapped(E);
    if (C)
      return new P(C, I, Q.cols);
  }
  getNullCell() {
...
class P {
  ...
  translateToString(A = !1, Q = 0, g = this._length) {
    const C = Math.max(0, Math.min(Q, this._length)), E = Math.max(C, Math.min(g, this._length));
    let I = "";
    for (let D = C; D < E; D++) {
...
const j = {
  ...
  extractModifiers(A) {
    let Q = U.NONE;
    return A.shiftKey && (Q |= U.SHIFT), A.ctrlKey && (Q |= U.CTRL), A.altKey && (Q |= U.ALT), A.metaKey && (Q |= U.SUPER), Q;
  }
  /**
  ...
  handleKeyDown(A) {
    ...
    if (g === U.NONE || g === U.SHIFT) {
      ...
      if (E !== null) {
        A.preventDefault(), this.onDataCallback(E), this.recordKeyDownData(E);
        return;
      }
    }
    const C = u.PRESS;
    try {
      if (this.getModeCallback) {
        ...
        this.encoder.setOption(p.CURSOR_KEY_APPLICATION, w);
      }
      const E = A.key.length === 1 && A.key.charCodeAt(0) < 128 ? A.key.toLowerCase() : void 0, I = this.encoder.encode({
        action: C,
        key: Q,
        mods: g,
        utf8: E
      ...
      console.warn("Failed to encode key:", A.code, E);
    }
  }
  /**
  ...
  handleBeforeInput(A) {
    ...
    if (C) {
      if (this.shouldIgnoreBeforeInput(C)) {
        A.preventDefault(), A.stopPropagation();
        return;
      }
      ...
      A.preventDefault(), A.stopPropagation(), this.onDataCallback(C), g && this.recordBeforeInputData(g);
    }
  }
  /**
  ...
  pixelToCell(A) {
    ...
    if (Q.width <= 0 || Q.height <= 0)
      return null;
    const C = A.clientX - g.left, E = A.clientY - g.top, I = Math.floor(C / Q.width) + 1, D = Math.floor(E / Q.height) + 1;
    return {
      col: Math.max(1, I),
      row: Math.max(1, D)
    };
   ...
   */
  encodeMouseSGR(A, Q, g, C, E) {
    return `\x1B[<${A + E};${Q};${g}${C ? "m" : "M"}`;
  }
  /**
   * Encode mouse event as X10/normal sequence (legacy format)
   * Format: \x1b[M<Btn+32><Col+32><Row+32>
   */
  encodeMouseX10(A, Q, g, C) {
    const E = A + C + 32, I = String.fromCharCode(Math.min(Q + 32, 255)), D = String.fromCharCode(Math.min(g + 32, 255));
    return `\x1B[M${String.fromCharCode(E)}${I}${D}`;
  }
  /**
   * Send mouse event to terminal
   */
  sendMouseEvent(A, Q, g, C, E) {
    var w, s;
    const I = this.getMouseModifiers(E), D = ((s = (w = this.mouseConfig) == null ? void 0 : w.hasSgrMouseMode) == null ? void 0 : s.call(w)) ?? !0;
    let o;
    if (D)
      o = this.encodeMouseSGR(A, Q, g, C, I);
    else {
      const t = C ? 3 : A;
      o = this.encodeMouseX10(t, Q, g, I);
    }
  ...
  handleMouseMove(A) {
    ...
    if (!C)
      return;
    let E = 32;
    this.mouseButtonsPressed & 1 ? E += 0 : this.mouseButtonsPressed & 2 ? E += 1 : this.mouseButtonsPressed & 4 && (E += 2), this.sendMouseEvent(E, C.col, C.row, !1, A);
  }
  /**
  ...
  shouldIgnoreBeforeInput(A) {
    if (!this.lastKeyDownData)
      return !1;
    const g = this.getNow() - this.lastKeyDownTime < K.BEFORE_INPUT_IGNORE_MS && this.lastKeyDownData === A;
  ...
  shouldIgnoreBeforeInputFromComposition(A) {
    if (!this.lastCompositionData)
      return !1;
    const g = this.getNow() - this.lastCompositionTime < K.BEFORE_INPUT_IGNORE_MS && this.lastCompositionData === A;
  ...
  shouldIgnoreCompositionEnd(A) {
    if (!this.lastBeforeInputData)
      return !1;
    const g = this.getNow() - this.lastBeforeInputTime < K.BEFORE_INPUT_IGNORE_MS && this.lastBeforeInputData === A;
  ...
  shouldIgnorePasteEvent(A, Q) {
    if (!this.lastPasteData || this.lastPasteSource === Q)
      return !1;
    const C = this.getNow() - this.lastPasteTime < K.BEFORE_INPUT_IGNORE_MS && this.lastPasteData === A;
    return C && (this.lastPasteData = null, this.lastPasteSource = null), C;
  }
  /**
...
class _ {
  ...
  async getLinkAt(A, Q) {
    ...
    if (!C)
      return;
    const E = C.getHyperlinkId();
    if (E > 0) {
      const I = `h${E}`;
      if (this.linkCache.has(I))
        return this.linkCache.get(I);
    }
    if (this.scannedRows.has(Q) || await this.scanRow(Q), E > 0) {
      const I = `h${E}`, D = this.linkCache.get(I);
      if (D)
        return D;
    }
    for (const I of this.linkCache.values())
      if (this.isPositionInLink(A, Q, I))
        return I;
  }
  /**
   ...
   */
  cacheLink(A) {
    const { start: Q } = A.range, g = this.terminal.buffer.active.getLine(Q.y);
    if (g) {
      const D = g.getCell(Q.x);
      if (!D) {
    ...
    const { start: C, end: E } = A.range, I = `r${C.y}:${C.x}-${E.x}`;
  ...
  isPositionInLink(A, Q, g) {
    const { start: C, end: E } = g.range;
    return Q < C.y || Q > E.y ? !1 : C.y === E.y ? A >= C.x && A <= E.x : Q === C.y ? A >= C.x : Q === E.y ? A <= E.x : !0;
  }
  /**
  ...
  invalidateRows(A, Q) {
    for (let C = A; C <= Q; C++)
      this.scannedRows.delete(C);
    const g = [];
    for (const [C, E] of this.linkCache.entries()) {
      const { start: I, end: D } = E.range;
      (I.y >= A && I.y <= Q || D.y >= A && D.y <= Q || I.y < A && D.y > Q) && g.push(C);
    }
    for (const C of g)
      this.linkCache.delete(C);
  }
  /**
   * Dispose and cleanup
   */
...
class $ {
   ...
   */
  provideLinks(A, Q) {
    const g = [], C = /* @__PURE__ */ new Set(), E = this.terminal.buffer.active.getLine(A);
    if (!E) {
      Q(void 0);
      return;
    }
    for (let I = 0; I < E.length; I++) {
      ...
      if (o === 0 || C.has(o))
        continue;
      C.add(o);
      const w = this.findLinkRange(o, A, I);
      if (!this.terminal.wasmTerm)
        continue;
      const s = this.terminal.wasmTerm.getHyperlinkUri(o);
      s && g.push({
        text: s,
        range: w,
        activate: (t) => {
   ...
   */
  findLinkRange(A, Q, g) {
    const C = this.terminal.buffer.active;
    let E = Q, I = g;
    for (; I > 0; ) {
      const s = C.getLine(E);
      if (!s)
...
const x = class l {
   ...
   */
  provideLinks(A, Q) {
    const g = [], C = this.terminal.buffer.active.getLine(A);
    if (!C) {
      Q(void 0);
      return;
    }
    const E = this.lineToText(C);
    l.URL_REGEX.lastIndex = 0;
    let I = l.URL_REGEX.exec(E);
    for (; I !== null; ) {
      let D = I[0];
      const o = I.index;
      let w = I.index + D.length - 1;
      const s = D.replace(l.TRAILING_PUNCTUATION, "");
  ...
  lineToText(A) {
    ...
    for (let g = 0; g < A.length; g++) {
      ...
      const E = C.getCodepoint();
      E === 0 || E < 32 ? Q.push(" ") : Q.push(String.fromCodePoint(E));
    }
    return Q.join("");
  }
  dispose() {
...
let AA = x;
function QA(B) {
  return B >= 9472 && B <= 9599;
}
const BA = {
  // Phase 1: Light lines
  9472: { up: "none", right: "light", down: "none", left: "light" },
  // ─ horizontal
  9474: { up: "light", right: "none", down: "light", left: "none" },
  // │ vertical
  9484: { up: "none", right: "light", down: "light", left: "none" },
  // ┌ down and right
  9488: { up: "none", right: "none", down: "light", left: "light" },
  // ┐ down and left
  9492: { up: "light", right: "light", down: "none", left: "none" },
  // └ up and right
  9496: { up: "light", right: "none", down: "none", left: "light" },
  // ┘ up and left
  9500: { up: "light", right: "light", down: "light", left: "none" },
  // ├ vertical and right
  9508: { up: "light", right: "none", down: "light", left: "light" },
  // ┤ vertical and left
  9516: { up: "none", right: "light", down: "light", left: "light" },
  // ┬ down and horizontal
  9524: { up: "light", right: "light", down: "none", left: "light" },
  // ┴ up and horizontal
  9532: { up: "light", right: "light", down: "light", left: "light" },
  // ┼ vertical and horizontal
  // Phase 2: Heavy lines
  9473: { up: "none", right: "heavy", down: "none", left: "heavy" },
  // ━ heavy horizontal
  9475: { up: "heavy", right: "none", down: "heavy", left: "none" },
  // ┃ heavy vertical
  9487: { up: "none", right: "heavy", down: "heavy", left: "none" },
  // ┏ heavy down and right
  9491: { up: "none", right: "none", down: "heavy", left: "heavy" },
  // ┓ heavy down and left
  9495: { up: "heavy", right: "heavy", down: "none", left: "none" },
  // ┗ heavy up and right
  9499: { up: "heavy", right: "none", down: "none", left: "heavy" },
  // ┛ heavy up and left
  9507: { up: "heavy", right: "heavy", down: "heavy", left: "none" },
  // ┣ heavy vertical and right
  9515: { up: "heavy", right: "none", down: "heavy", left: "heavy" },
  // ┫ heavy vertical and left
  9523: { up: "none", right: "heavy", down: "heavy", left: "heavy" },
  // ┳ heavy down and horizontal
  9531: { up: "heavy", right: "heavy", down: "none", left: "heavy" },
  // ┻ heavy up and horizontal
  9547: { up: "heavy", right: "heavy", down: "heavy", left: "heavy" },
  // ╋ heavy vertical and horizontal
  // Phase 3: Double lines
  9552: { up: "none", right: "double", down: "none", left: "double" },
  // ═ double horizontal
  9553: { up: "double", right: "none", down: "double", left: "none" },
  // ║ double vertical
  9556: { up: "none", right: "double", down: "double", left: "none" },
  // ╔ double down and right
  9559: { up: "none", right: "none", down: "double", left: "double" },
  // ╗ double down and left
  9562: { up: "double", right: "double", down: "none", left: "none" },
  // ╚ double up and right
  9565: { up: "double", right: "none", down: "none", left: "double" },
  // ╝ double up and left
  9568: { up: "double", right: "double", down: "double", left: "none" },
  // ╠ double vertical and right
  9571: { up: "double", right: "none", down: "double", left: "double" },
  // ╣ double vertical and left
  9574: { up: "none", right: "double", down: "double", left: "double" },
  // ╦ double down and horizontal
  9577: { up: "double", right: "double", down: "none", left: "double" },
  // ╩ double up and horizontal
  9580: { up: "double", right: "double", down: "double", left: "double" },
  // ╬ double vertical and horizontal
  // Mixed light/heavy
  9485: { up: "none", right: "heavy", down: "light", left: "none" },
  // ┍ down light and right heavy
  9486: { up: "none", right: "light", down: "heavy", left: "none" },
  // ┎ down heavy and right light
  9489: { up: "none", right: "none", down: "light", left: "heavy" },
  // ┑ down light and left heavy
  9490: { up: "none", right: "none", down: "heavy", left: "light" },
  // ┒ down heavy and left light
  9493: { up: "light", right: "heavy", down: "none", left: "none" },
  // ┕ up light and right heavy
  9494: { up: "heavy", right: "light", down: "none", left: "none" },
  // ┖ up heavy and right light
  9497: { up: "light", right: "none", down: "none", left: "heavy" },
  // ┙ up light and left heavy
  9498: { up: "heavy", right: "none", down: "none", left: "light" },
  // ┚ up heavy and left light
  // T-junctions with mixed weights
  9501: { up: "light", right: "heavy", down: "light", left: "none" },
  // ┝ vertical light and right heavy
  9502: { up: "heavy", right: "light", down: "light", left: "none" },
  // ┞ up heavy and right down light
  9503: { up: "light", right: "light", down: "heavy", left: "none" },
  // ┟ down heavy and right up light
  9504: { up: "heavy", right: "light", down: "heavy", left: "none" },
  // ┠ vertical heavy and right light
  9505: { up: "heavy", right: "heavy", down: "light", left: "none" },
  // ┡ down light and right up heavy
  9506: { up: "light", right: "heavy", down: "heavy", left: "none" },
  // ┢ up light and right down heavy
  9509: { up: "light", right: "none", down: "light", left: "heavy" },
  // ┥ vertical light and left heavy
  9510: { up: "heavy", right: "none", down: "light", left: "light" },
  // ┦ up heavy and left down light
  9511: { up: "light", right: "none", down: "heavy", left: "light" },
  // ┧ down heavy and left up light
  9512: { up: "heavy", right: "none", down: "heavy", left: "light" },
  // ┨ vertical heavy and left light
  9513: { up: "heavy", right: "none", down: "light", left: "heavy" },
  // ┩ down light and left up heavy
  9514: { up: "light", right: "none", down: "heavy", left: "heavy" },
  // ┪ up light and left down heavy
  9517: { up: "none", right: "light", down: "light", left: "heavy" },
  // ┭ left heavy and right down light
  9518: { up: "none", right: "heavy", down: "light", left: "light" },
  // ┮ right heavy and left down light
  9519: { up: "none", right: "heavy", down: "light", left: "heavy" },
  // ┯ down light and horizontal heavy
  9520: { up: "none", right: "light", down: "heavy", left: "light" },
  // ┰ down heavy and horizontal light
  9521: { up: "none", right: "light", down: "heavy", left: "heavy" },
  // ┱ right light and left down heavy
  9522: { up: "none", right: "heavy", down: "heavy", left: "light" },
  // ┲ left light and right down heavy
  9525: { up: "light", right: "light", down: "none", left: "heavy" },
  // ┵ left heavy and right up light
  9526: { up: "light", right: "heavy", down: "none", left: "light" },
  // ┶ right heavy and left up light
  9527: { up: "light", right: "heavy", down: "none", left: "heavy" },
  // ┷ up light and horizontal heavy
  9528: { up: "heavy", right: "light", down: "none", left: "light" },
  // ┸ up heavy and horizontal light
  9529: { up: "heavy", right: "light", down: "none", left: "heavy" },
  // ┹ right light and left up heavy
  9530: { up: "heavy", right: "heavy", down: "none", left: "light" },
  // ┺ left light and right up heavy
  // Cross junctions with mixed weights
  9533: { up: "light", right: "light", down: "light", left: "heavy" },
  // ┽ left heavy and right vertical light
  9534: { up: "light", right: "heavy", down: "light", left: "light" },
  // ┾ right heavy and left vertical light
  9535: { up: "light", right: "heavy", down: "light", left: "heavy" },
  // ┿ vertical light and horizontal heavy
  9536: { up: "heavy", right: "light", down: "light", left: "light" },
  // ╀ up heavy and down horizontal light
  9537: { up: "light", right: "light", down: "heavy", left: "light" },
  // ╁ down heavy and up horizontal light
  9538: { up: "heavy", right: "light", down: "heavy", left: "light" },
  // ╂ vertical heavy and horizontal light
  9539: { up: "heavy", right: "light", down: "light", left: "heavy" },
  // ╃ left up heavy and right down light
  9540: { up: "heavy", right: "heavy", down: "light", left: "light" },
  // ╄ right up heavy and left down light
  9541: { up: "light", right: "light", down: "heavy", left: "heavy" },
  // ╅ left down heavy and right up light
  9542: { up: "light", right: "heavy", down: "heavy", left: "light" },
  // ╆ right down heavy and left up light
  9543: { up: "heavy", right: "heavy", down: "light", left: "heavy" },
  // ╇ down light and up horizontal heavy
  9544: { up: "light", right: "heavy", down: "heavy", left: "heavy" },
  // ╈ up light and down horizontal heavy
  9545: { up: "heavy", right: "light", down: "heavy", left: "heavy" },
  // ╉ right light and left vertical heavy
  9546: { up: "heavy", right: "heavy", down: "heavy", left: "light" },
  // ╊ left light and right vertical heavy
  // Mixed double/light
  9554: { up: "none", right: "double", down: "light", left: "none" },
  // ╒ down light and right double
  9555: { up: "none", right: "light", down: "double", left: "none" },
  // ╓ down double and right light
  9557: { up: "none", right: "none", down: "light", left: "double" },
  // ╕ down light and left double
  9558: { up: "none", right: "none", down: "double", left: "light" },
  // ╖ down double and left light
  9560: { up: "light", right: "double", down: "none", left: "none" },
  // ╘ up light and right double
  9561: { up: "double", right: "light", down: "none", left: "none" },
  // ╙ up double and right light
  9563: { up: "light", right: "none", down: "none", left: "double" },
  // ╛ up light and left double
  9564: { up: "double", right: "none", down: "none", left: "light" },
  // ╜ up double and left light
  9566: { up: "light", right: "double", down: "light", left: "none" },
  // ╞ vertical light and right double
  9567: { up: "double", right: "light", down: "double", left: "none" },
  // ╟ vertical double and right light
  9569: { up: "light", right: "none", down: "light", left: "double" },
  // ╡ vertical light and left double
  9570: { up: "double", right: "none", down: "double", left: "light" },
  // ╢ vertical double and left light
  9572: { up: "none", right: "double", down: "light", left: "double" },
  // ╤ down light and horizontal double
  9573: { up: "none", right: "light", down: "double", left: "light" },
  // ╥ down double and horizontal light
  9575: { up: "light", right: "double", down: "none", left: "double" },
  // ╧ up light and horizontal double
  9576: { up: "double", right: "light", down: "none", left: "light" },
  // ╨ up double and horizontal light
  9578: { up: "light", right: "double", down: "light", left: "double" },
  // ╪ vertical light and horizontal double
  9579: { up: "double", right: "light", down: "double", left: "light" },
  // ╫ vertical double and horizontal light
  // Dashed lines (Phase 4)
  9476: { up: "none", right: "light", down: "none", left: "light" },
  // ┄ triple dash horizontal (treat as light)
  9477: { up: "none", right: "heavy", down: "none", left: "heavy" },
  // ┅ triple dash horizontal heavy
  9478: { up: "light", right: "none", down: "light", left: "none" },
  // ┆ triple dash vertical
  9479: { up: "heavy", right: "none", down: "heavy", left: "none" },
  // ┇ triple dash vertical heavy
  9480: { up: "none", right: "light", down: "none", left: "light" },
  // ┈ quadruple dash horizontal
  9481: { up: "none", right: "heavy", down: "none", left: "heavy" },
  // ┉ quadruple dash horizontal heavy
  9482: { up: "light", right: "none", down: "light", left: "none" },
  // ┊ quadruple dash vertical
  9483: { up: "heavy", right: "none", down: "heavy", left: "none" },
  // ┋ quadruple dash vertical heavy
  9548: { up: "none", right: "light", down: "none", left: "light" },
  // ╌ double dash horizontal
  9549: { up: "none", right: "heavy", down: "none", left: "heavy" },
  // ╍ double dash horizontal heavy
  9550: { up: "light", right: "none", down: "light", left: "none" },
  // ╎ double dash vertical
  9551: { up: "heavy", right: "none", down: "heavy", left: "none" },
  // ╏ double dash vertical heavy
  // Rounded corners (Phase 4) - treat as light lines
  9581: { up: "none", right: "light", down: "light", left: "none" },
  // ╭ arc down and right
  9582: { up: "none", right: "none", down: "light", left: "light" },
  // ╮ arc down and left
  9583: { up: "light", right: "none", down: "none", left: "light" },
  // ╯ arc up and left
  9584: { up: "light", right: "light", down: "none", left: "none" },
  // ╰ arc up and right
  // Half lines
  9588: { up: "none", right: "none", down: "none", left: "light" },
  // ╴ left light
  9589: { up: "light", right: "none", down: "none", left: "none" },
  // ╵ up light
  9590: { up: "none", right: "light", down: "none", left: "none" },
  // ╶ right light
  9591: { up: "none", right: "none", down: "light", left: "none" },
  // ╷ down light
  9592: { up: "none", right: "none", down: "none", left: "heavy" },
  // ╸ left heavy
  9593: { up: "heavy", right: "none", down: "none", left: "none" },
  // ╹ up heavy
  9594: { up: "none", right: "heavy", down: "none", left: "none" },
  // ╺ right heavy
  9595: { up: "none", right: "none", down: "heavy", left: "none" },
  // ╻ down heavy
  9596: { up: "none", right: "heavy", down: "none", left: "light" },
  // ╼ left light and right heavy
  9597: { up: "light", right: "none", down: "heavy", left: "none" },
  // ╽ up light and down heavy
  9598: { up: "none", right: "light", down: "none", left: "heavy" },
  // ╾ left heavy and right light
  9599: { up: "heavy", right: "none", down: "light", left: "none" }
  // ╿ up heavy and down light
};
function gA(B) {
  return BA[B] ?? null;
}
function EA(B) {
  return B >= 9600 && B <= 9631;
}
function CA(B) {
  switch (B) {
    case 9600:
      return { type: "upper", eighths: 4 };
    case 9601:
      return { type: "lower", eighths: 1 };
    case 9602:
      return { type: "lower", eighths: 2 };
    case 9603:
      return { type: "lower", eighths: 3 };
    case 9604:
      return { type: "lower", eighths: 4 };
    case 9605:
      return { type: "lower", eighths: 5 };
    case 9606:
      return { type: "lower", eighths: 6 };
    case 9607:
      return { type: "lower", eighths: 7 };
    case 9608:
      return { type: "full" };
    case 9609:
      return { type: "left", eighths: 7 };
    case 9610:
      return { type: "left", eighths: 6 };
    case 9611:
      return { type: "left", eighths: 5 };
    case 9612:
      return { type: "left", eighths: 4 };
    case 9613:
      return { type: "left", eighths: 3 };
    case 9614:
      return { type: "left", eighths: 2 };
    case 9615:
      return { type: "left", eighths: 1 };
    case 9616:
      return { type: "right", eighths: 4 };
    case 9617:
      return { type: "shade", density: 0.25 };
    case 9618:
      return { type: "shade", density: 0.5 };
    case 9619:
      return { type: "shade", density: 0.75 };
    case 9620:
      return { type: "upper", eighths: 1 };
    case 9621:
      return { type: "right", eighths: 1 };
    case 9622:
      return { type: "quadrants", tl: !1, tr: !1, bl: !0, br: !1 };
    case 9623:
      return { type: "quadrants", tl: !1, tr: !1, bl: !1, br: !0 };
    case 9624:
      return { type: "quadrants", tl: !0, tr: !1, bl: !1, br: !1 };
    case 9625:
      return { type: "quadrants", tl: !0, tr: !1, bl: !0, br: !0 };
    case 9626:
      return { type: "quadrants", tl: !0, tr: !1, bl: !1, br: !0 };
    case 9627:
      return { type: "quadrants", tl: !0, tr: !0, bl: !0, br: !1 };
    case 9628:
      return { type: "quadrants", tl: !0, tr: !0, bl: !1, br: !0 };
    case 9629:
      return { type: "quadrants", tl: !1, tr: !0, bl: !1, br: !1 };
    case 9630:
      return { type: "quadrants", tl: !1, tr: !0, bl: !0, br: !1 };
    case 9631:
      return { type: "quadrants", tl: !1, tr: !0, bl: !0, br: !0 };
    default:
      return null;
  }
}
function IA(B) {
  return B >= 10240 && B <= 10495;
}
function DA(B) {
  const A = B - 10240;
  return [
    ...
    // Dot 1 (top-left)
    (A & 2) !== 0,
    // Dot 2 (middle-upper-left)
    (A & 4) !== 0,
    // Dot 3 (middle-lower-left)
    (A & 8) !== 0,
    // Dot 4 (top-right)
    (A & 16) !== 0,
    // Dot 5 (middle-upper-right)
    (A & 32) !== 0,
    // Dot 6 (middle-lower-right)
    (A & 64) !== 0,
    // Dot 7 (bottom-left)
    (A & 128) !== 0
    // Dot 8 (bottom-right)
...
function eA(B) {
  return B >= 129852 && B <= 129931;
}
function MA(B) {
  if (B >= 129852 && B <= 129855)
    return { type: "diagonal", from: "bl", size: ["small", "half", "half", "large"][B - 129852] || "half" };
  if (B >= 129856 && B <= 129859)
    return { type: "diagonal", from: "br", size: ["small", "half", "half", "large"][B - 129856] || "half" };
  if (B >= 129860 && B <= 129863)
    return { type: "diagonal", from: "tl", size: ["small", "half", "half", "large"][B - 129860] || "half" };
  if (B >= 129864 && B <= 129867)
    return { type: "diagonal", from: "tr", size: ["small", "half", "half", "large"][B - 129864] || "half" };
  if (B >= 129868 && B <= 129895) {
    const A = B - 129868;
    return { type: "diagonal", from: ["bl", "br", "tl", "tr"][A % 4], size: "half" };
  }
  if (B >= 129896 && B <= 129903)
    return { type: "diagonal", from: ["bl", "br", "tl", "tr"][(B - 129896) % 4], size: "large" };
  if (B >= 129904 && B <= 129909)
    return { type: "half-scanline", position: "upper", part: "left" };
  if (B >= 129910 && B <= 129915)
    return { type: "half-scanline", position: "upper", part: "right" };
  if (B >= 129916 && B <= 129931) {
    const A = B - 129916;
    return { type: "half-scanline", position: A < 8 ? "upper" : "lower", part: A % 2 === 0 ? "left" : "right" };
  }
  return null;
}
function aA(B) {
  return B >= 118016 && B <= 118245;
}
function kA(B) {
  const A = B - 118016;
  return [
...
function NA(B) {
  return B >= 129936 && B <= 129967;
}
function cA(B) {
  return B >= 9581 && B <= 9584;
}
function GA(B) {
  switch (B) {
    case 9581:
...
function FA(B) {
  return B >= 9476 && B <= 9483 || // Triple/quadruple dash
  B >= 9548 && B <= 9551;
}
function rA(B) {
  switch (B) {
    case 9476:
      return { direction: "horizontal", weight: "light", dashes: 3 };
    case 9477:
      return { direction: "horizontal", weight: "heavy", dashes: 3 };
    case 9478:
      return { direction: "vertical", weight: "light", dashes: 3 };
    case 9479:
      return { direction: "vertical", weight: "heavy", dashes: 3 };
    case 9480:
      return { direction: "horizontal", weight: "light", dashes: 4 };
    case 9481:
      return { direction: "horizontal", weight: "heavy", dashes: 4 };
    case 9482:
      return { direction: "vertical", weight: "light", dashes: 4 };
    case 9483:
      return { direction: "vertical", weight: "heavy", dashes: 4 };
    case 9548:
      return { direction: "horizontal", weight: "light", dashes: 2 };
    case 9549:
      return { direction: "horizontal", weight: "heavy", dashes: 2 };
    case 9550:
      return { direction: "vertical", weight: "light", dashes: 2 };
    case 9551:
      return { direction: "vertical", weight: "heavy", dashes: 2 };
    default:
      return null;
  }
}
...
class nA {
  constructor(A, Q = {}) {
    this.cursorVisible = !0, this.lastCursorPosition = { x: 0, y: 0 }, this.lastViewportY = 0, this.currentBuffer = null, this.currentSelectionCoords = null, this.hoveredHyperlinkId = 0, this.previousHoveredHyperlinkId = 0, this.hoveredLinkRange = null, this.previousHoveredLinkRange = null, this.cursorSuppressed = !1, this.canvas = A;
  ...
  measureFont() {
    ...
    const g = Q.measureText("M"), C = g.fontBoundingBoxAscent ?? g.actualBoundingBoxAscent ?? this.fontSize * 0.8, E = g.fontBoundingBoxDescent ?? g.actualBoundingBoxDescent ?? this.fontSize * 0.2, I = Math.ceil(g.width), D = Math.ceil(C + E), o = Math.ceil(C);
    return { width: I, height: D, baseline: o };
  }
  /**
   ...
   */
  resize(A, Q) {
    const g = A * this.metrics.width, C = Q * this.metrics.height;
    this.canvas.style.width = `${g}px`, this.canvas.style.height = `${C}px`, this.canvas.width = g * this.devicePixelRatio, this.canvas.height = C * this.devicePixelRatio, this.ctx.scale(this.devicePixelRatio, this.devicePixelRatio), this.ctx.textBaseline = "alphabetic", this.ctx.textAlign = "left", this.ctx.fillStyle = this.theme.background, this.ctx.fillRect(0, 0, g, C);
  }
  // ==========================================================================
  // Main Rendering
  // ==========================================================================
   ...
   */
  render(A, Q = !1, g = 0, C, E = 1) {
    var r;
    this.currentBuffer = A;
    const I = A.getCursor(), D = A.getDimensions(), o = C ? C.getScrollbackLength() : 0;
    ...
    if (s || this.cursorBlink) {
      if (!Q && !A.isRowDirty(I.y)) {
        const k = A.getLine(I.y);
        k && this.renderLine(k, I.y, D.cols);
      }
      if (s && this.lastCursorPosition.y !== I.y && !Q && !A.isRowDirty(this.lastCursorPosition.y)) {
        const k = A.getLine(this.lastCursorPosition.y);
        k && this.renderLine(k, this.lastCursorPosition.y, D.cols);
      }
    }
    ...
    if (this.currentSelectionCoords = t ? this.selectionManager.getSelectionCoords() : null, this.currentSelectionCoords) {
      const k = this.currentSelectionCoords;
      for (let F = k.startRow; F <= k.endRow; F++)
        e.add(F);
    }
    if (this.selectionManager) {
      const k = this.selectionManager.getDirtySelectionRows();
      if (k.size > 0) {
        for (const F of k)
          e.add(F);
    ...
    if (a) {
      for (let k = 0; k < D.rows; k++) {
        ...
        if (g > 0)
          if (k < g && C) {
            const n = o - Math.floor(g) + k;
            F = C.getScrollbackLine(n);
          } else {
            const n = k - Math.floor(g);
            F = A.getLine(n);
          }
        else
          F = A.getLine(k);
    ...
    if (h) {
      if (this.previousHoveredLinkRange)
        for (let k = this.previousHoveredLinkRange.startY; k <= this.previousHoveredLinkRange.endY; k++)
          M.add(k);
      if (this.hoveredLinkRange)
        for (let k = this.hoveredLinkRange.startY; k <= this.hoveredLinkRange.endY; k++)
          M.add(k);
      this.previousHoveredLinkRange = this.hoveredLinkRange;
    }
    const c = /* @__PURE__ */ new Set();
    for (let k = 0; k < D.rows; k++)
      (g > 0 ? !0 : Q || A.isRowDirty(k) || e.has(k) || M.has(k)) && (c.add(k), k > 0 && c.add(k - 1), k < D.rows - 1 && c.add(k + 1));
    for (let k = 0; k < D.rows; k++) {
      if (!c.has(k))
        continue;
      let F = null;
      if (g > 0)
        if (k < g && C) {
          ...
          const n = g > 0 ? k - Math.floor(g) : k;
          F = A.getLine(n);
        }
      else
        F = A.getLine(k);
      F && this.renderLine(F, k, D.cols);
    }
    g === 0 && I.visible && this.cursorVisible && !this.cursorSuppressed && this.renderCursor(I.x, I.y), C && E > 0 && this.renderScrollbar(g, o, D.rows, E), this.lastCursorPosition = { x: I.x, y: I.y }, A.clearDirty();
  }
  /**
  ...
  renderCellBackground(A, Q, g) {
    const C = Q * this.metrics.width, E = g * this.metrics.height, I = this.metrics.width * A.width;
    if (this.isInSelection(Q, g)) {
      this.ctx.fillStyle = this.theme.selectionBackground, this.ctx.fillRect(C, E, I, this.metrics.height);
      return;
    }
    let o = A.bg_r, w = A.bg_g, s = A.bg_b;
    A.flags & G.INVERSE && (o = A.fg_r, w = A.fg_g, s = A.fg_b), o === 0 && w === 0 && s === 0 || (this.ctx.fillStyle = this.rgbToCSS(o, w, s), this.ctx.fillRect(C, E, I, this.metrics.height));
  }
  /**
   * Render a cell's text and decorations (Pass 2 of two-pass rendering)
   * Selection foreground color is applied here to match the selection background.
   */
  renderCellText(A, Q, g) {
    var a;
    const C = Q * this.metrics.width, E = g * this.metrics.height, I = this.metrics.width * A.width;
    if (A.flags & G.INVISIBLE)
      return;
    const D = this.isInSelection(Q, g);
    let o = "";
    if (A.flags & G.ITALIC && (o += "italic "), A.flags & G.BOLD && (o += "bold "), this.ctx.font = `${o}${this.fontSize}px ${this.fontFamily}`, D)
      ...
      let h = A.fg_r, c = A.fg_g, N = A.fg_b;
      A.flags & G.INVERSE && (h = A.bg_r, c = A.bg_g, N = A.bg_b), this.ctx.fillStyle = this.rgbToCSS(h, c, N);
    }
    A.flags & G.FAINT && (this.ctx.globalAlpha = 0.5);
    const w = C, s = E + this.metrics.baseline, t = A.codepoint || 32, e = this.ctx.fillStyle;
    if (QA(t)) {
      this.drawBoxChar(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (EA(t)) {
      this.drawBlockChar(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (IA(t)) {
      this.drawBrailleChar(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (oA(t)) {
      this.drawSextantChar(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (wA(t)) {
      this.drawCornerTriangle(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (sA(t)) {
      this.drawPowerlineChar(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (eA(t)) {
      this.drawLegacyWedge(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (aA(t)) {
      this.drawOctant(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (NA(t)) {
      this.drawSmoothMosaic(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (cA(t)) {
      this.drawRoundedCorner(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    if (FA(t)) {
      this.drawDashedLine(t, C, E, e), A.flags & G.FAINT && (this.ctx.globalAlpha = 1);
      return;
    }
    let M;
    if (A.grapheme_len > 0 && ((a = this.currentBuffer) != null && a.getGraphemeString) ? M = this.currentBuffer.getGraphemeString(g, Q) : M = String.fromCodePoint(t), this.ctx.fillText(M, w, s), A.flags & G.FAINT && (this.ctx.globalAlpha = 1), A.flags & G.UNDERLINE) {
      const h = E + this.metrics.baseline + 2;
      this.ctx.strokeStyle = this.ctx.fillStyle, this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, h), this.ctx.lineTo(C + I, h), this.ctx.stroke();
    }
    if (A.flags & G.STRIKETHROUGH) {
      const h = E + this.metrics.height / 2;
      this.ctx.strokeStyle = this.ctx.fillStyle, this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, h), this.ctx.lineTo(C + I, h), this.ctx.stroke();
    }
    if (A.hyperlink_id > 0 && A.hyperlink_id === this.hoveredHyperlinkId) {
      const c = E + this.metrics.baseline + 2;
      this.ctx.strokeStyle = "#4A90E2", this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, c), this.ctx.lineTo(C + I, c), this.ctx.stroke();
    }
    if (this.hoveredLinkRange) {
      const h = this.hoveredLinkRange;
      if (g === h.startY && Q >= h.startX && (g < h.endY || Q <= h.endX) || g > h.startY && g < h.endY || g === h.endY && Q <= h.endX && (g > h.startY || Q >= h.startX)) {
        const N = E + this.metrics.baseline + 2;
        this.ctx.strokeStyle = "#4A90E2", this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, N), this.ctx.lineTo(C + I, N), this.ctx.stroke();
      }
    }
  }
  // ==========================================================================
  ...
  drawBoxChar(A, Q, g, C) {
    ...
    if (!E)
      return;
    const I = this.metrics.width, D = this.metrics.height, o = Math.max(1, Math.round(D / 12)), w = Math.max(2, Math.round(D / 6)), s = Math.max(2, Math.round(D / 8)), t = Math.max(1, Math.round(D / 16)), e = Q + I / 2, M = g + D / 2;
    ...
    if (E.left !== "none" || E.right !== "none")
      if (E.left === "double" || E.right === "double") {
        const h = (s + t) / 2;
        E.left !== "none" && (this.ctx.fillRect(
          Q,
          M - h - t / 2,
          I / 2 + (E.right !== "none" ? t / 2 : 0),
          t
        ), this.ctx.fillRect(
          Q,
          M + h - t / 2,
          I / 2 + (E.right !== "none" ? t / 2 : 0),
          t
        )), E.right !== "none" && (this.ctx.fillRect(
          e - (E.left !== "none" ? t / 2 : 0),
          M - h - t / 2,
          I / 2 + (E.left !== "none" ? t / 2 : 0),
          t
        ), this.ctx.fillRect(
          e - (E.left !== "none" ? t / 2 : 0),
          M + h - t / 2,
        ...
        const h = a(E.left), c = a(E.right);
        if (E.left !== "none" && E.right !== "none" && E.left === E.right) {
          const N = h;
          this.ctx.fillRect(
            Q,
            Math.round(M - N / 2),
          ...
          if (E.left !== "none") {
            const N = h, r = E.right !== "none" ? N / 2 : 0;
            this.ctx.fillRect(
              Q,
              Math.round(M - N / 2),
              Math.ceil(I / 2 + r),
    ...
    if (E.up !== "none" || E.down !== "none")
      if (E.up === "double" || E.down === "double") {
        const h = (s + t) / 2;
        E.up !== "none" && (this.ctx.fillRect(
          Math.round(e - h - t / 2),
          g,
          t,
          Math.ceil(D / 2 + t / 2)
        ...
        const h = a(E.up), c = a(E.down);
        if (E.up !== "none" && E.down !== "none" && E.up === E.down) {
          const N = h;
          this.ctx.fillRect(
            Math.round(e - N / 2),
            g,
   ...
   * @param fgColor The foreground color to use
   */
  drawBlockChar(A, Q, g, C) {
    const E = CA(A);
    if (!E)
      return;
    const I = this.metrics.width, D = this.metrics.height;
    switch (this.ctx.fillStyle = C, E.type) {
      case "full":
        this.ctx.fillRect(Q, g, I, D);
        break;
      case "upper":
        const o = Math.round(D * E.eighths / 8);
        this.ctx.fillRect(Q, g, I, o);
        break;
      case "lower":
        const w = Math.round(D * E.eighths / 8);
        this.ctx.fillRect(Q, g + D - w, I, w);
        break;
      case "left":
        const s = Math.round(I * E.eighths / 8);
        this.ctx.fillRect(Q, g, s, D);
        break;
      case "right":
        const t = Math.round(I * E.eighths / 8);
        this.ctx.fillRect(Q + I - t, g, t, D);
        break;
      case "quadrants":
        const e = Math.ceil(I / 2), M = Math.ceil(D / 2), a = Q + Math.floor(I / 2), h = g + Math.floor(D / 2);
        E.tl && this.ctx.fillRect(Q, g, e, M), E.tr && this.ctx.fillRect(a, g, e, M), E.bl && this.ctx.fillRect(Q, h, e, M), E.br && this.ctx.fillRect(a, h, e, M);
        break;
      case "shade":
        this.ctx.globalAlpha = E.density, this.ctx.fillRect(Q, g, I, D), this.ctx.globalAlpha = 1;
        break;
    }
  }
  /**
   ...
   * @param fgColor The foreground color to use
   */
  drawBrailleChar(A, Q, g, C) {
    const E = DA(A), I = this.metrics.width, D = this.metrics.height, o = I * 0.15, w = D * 0.1, s = I - o * 2, t = D - w * 2, e = Math.max(1, Math.min(s / 4, t / 8) * 0.9), M = s, a = t / 3, h = Q + o + e, c = g + w + e;
    this.ctx.fillStyle = C;
    const N = [
      [0, 0],
      // Dot 1: top-left
    ...
    for (let r = 0; r < 8; r++)
      if (E[r]) {
        const [k, F] = N[r], n = h + k * M, L = c + F * a;
        this.ctx.beginPath(), this.ctx.arc(n, L, e, 0, Math.PI * 2), this.ctx.fill();
      }
  }
  /**
   ...
   * @param fgColor The foreground color to use
   */
  drawSextantChar(A, Q, g, C) {
    const E = iA(A), I = this.metrics.width, D = this.metrics.height, o = Math.ceil(I / 2), w = Math.ceil(D / 3);
    this.ctx.fillStyle = C;
    const s = [
      [0, 0],
      // Segment 0: top-left
    ...
    for (let t = 0; t < 6; t++)
      if (E[t]) {
        const [e, M] = s[t], a = Q + e * Math.floor(I / 2), h = g + M * Math.floor(D / 3), c = e === 1 ? I - Math.floor(I / 2) : o, N = M === 2 ? D - Math.floor(D / 3) * 2 : w;
        this.ctx.fillRect(a, h, c, N);
      }
  }
  /**
   ...
   * @param fgColor The foreground color to use
   */
  drawCornerTriangle(A, Q, g, C) {
    const E = tA(A);
    if (!E)
      return;
    const I = this.metrics.width, D = this.metrics.height;
    switch (this.ctx.fillStyle = C, this.ctx.beginPath(), E) {
      case "lower-right":
        this.ctx.moveTo(Q + I, g), this.ctx.lineTo(Q + I, g + D), this.ctx.lineTo(Q, g + D);
        break;
      case "lower-left":
        this.ctx.moveTo(Q, g), this.ctx.lineTo(Q + I, g + D), this.ctx.lineTo(Q, g + D);
        break;
      case "upper-left":
        this.ctx.moveTo(Q, g), this.ctx.lineTo(Q + I, g), this.ctx.lineTo(Q, g + D);
        break;
      case "upper-right":
        this.ctx.moveTo(Q, g), this.ctx.lineTo(Q + I, g), this.ctx.lineTo(Q + I, g + D);
        break;
    }
  ...
  drawPowerlineChar(A, Q, g, C) {
    ...
    switch (this.ctx.fillStyle = C, this.ctx.beginPath(), E) {
      case "right":
        this.ctx.moveTo(Q, g), this.ctx.lineTo(Q + I, g + D / 2), this.ctx.lineTo(Q, g + D);
        break;
      case "left":
        this.ctx.moveTo(Q + I, g), this.ctx.lineTo(Q, g + D / 2), this.ctx.lineTo(Q + I, g + D);
        break;
      case "up":
        this.ctx.moveTo(Q, g + D), this.ctx.lineTo(Q + I / 2, g), this.ctx.lineTo(Q + I, g + D);
        break;
      case "down":
        this.ctx.moveTo(Q, g), this.ctx.lineTo(Q + I / 2, g + D), this.ctx.lineTo(Q + I, g);
        break;
    }
    this.ctx.closePath(), this.ctx.fill();
  }
   ...
   * @param fgColor The foreground color to use
   */
  drawLegacyWedge(A, Q, g, C) {
    const E = MA(A);
    if (!E)
      return;
    const I = this.metrics.width, D = this.metrics.height;
    if (this.ctx.fillStyle = C, this.ctx.beginPath(), E.type === "diagonal") {
      let o;
      switch (E.size) {
        case "small":
          o = 0.33;
      ...
      switch (E.from) {
        case "bl":
          this.ctx.moveTo(Q, g + D), this.ctx.lineTo(Q + I * o, g + D), this.ctx.lineTo(Q, g + D * (1 - o));
          break;
        case "br":
          this.ctx.moveTo(Q + I, g + D), this.ctx.lineTo(Q + I * (1 - o), g + D), this.ctx.lineTo(Q + I, g + D * (1 - o));
          break;
        case "tl":
          this.ctx.moveTo(Q, g), this.ctx.lineTo(Q + I * o, g), this.ctx.lineTo(Q, g + D * o);
          break;
        case "tr":
          this.ctx.moveTo(Q + I, g), this.ctx.lineTo(Q + I * (1 - o), g), this.ctx.lineTo(Q + I, g + D * o);
          break;
      }
    } else if (E.type === "half-scanline") {
      const o = I / 2, w = D / 2, s = E.part === "left" ? Q : Q + o, t = E.position === "upper" ? g : g + w;
      this.ctx.fillRect(s, t, o, w);
      return;
    }
    this.ctx.closePath(), this.ctx.fill();
  }
   ...
   */
  drawOctant(A, Q, g, C) {
    const E = kA(A), I = this.metrics.width, D = this.metrics.height, o = Math.ceil(I / 2), w = Math.ceil(D / 4);
    this.ctx.fillStyle = C;
    const s = [
      [0, 0],
      [1, 0],
    ...
    for (let t = 0; t < 8; t++)
      if (E[t]) {
        const [e, M] = s[t], a = Q + e * Math.floor(I / 2), h = g + M * Math.floor(D / 4), c = e === 1 ? I - Math.floor(I / 2) : o, N = M === 3 ? D - Math.floor(D / 4) * 3 : w;
        this.ctx.fillRect(a, h, c, N);
      }
  }
  /**
   ...
   * @param fgColor The foreground color to use
   */
  drawSmoothMosaic(A, Q, g, C) {
    const E = this.metrics.width, I = this.metrics.height;
    this.ctx.fillStyle = C;
    const D = A - 129936;
    if (this.ctx.beginPath(), D < 8)
      switch (["bl", "br", "tl", "tr"][D % 4]) {
        case "bl":
          this.ctx.moveTo(Q, g + I), this.ctx.lineTo(Q + E, g + I), this.ctx.lineTo(Q, g);
          break;
        case "br":
          this.ctx.moveTo(Q + E, g + I), this.ctx.lineTo(Q, g + I), this.ctx.lineTo(Q + E, g);
          break;
        case "tl":
          this.ctx.moveTo(Q, g), this.ctx.lineTo(Q + E, g), this.ctx.lineTo(Q, g + I);
          break;
        case "tr":
          this.ctx.moveTo(Q + E, g), this.ctx.lineTo(Q, g), this.ctx.lineTo(Q + E, g + I);
          break;
      }
    else {
      this.ctx.fillRect(Q, g, E / 2, I / 2), this.ctx.fillRect(Q + E / 2, g + I / 2, E / 2, I / 2);
      return;
    }
    this.ctx.closePath(), this.ctx.fill();
  }
   ...
   * @param fgColor The foreground color to use
   */
  drawRoundedCorner(A, Q, g, C) {
    const E = this.metrics.width, I = this.metrics.height, D = Math.max(1, Math.round(I / 12)), o = Math.round(Q + E / 2), w = Math.round(g + I / 2), s = Math.min(E, I) / 2 - D / 2;
    switch (this.ctx.strokeStyle = C, this.ctx.lineWidth = D, this.ctx.lineCap = "square", this.ctx.beginPath(), GA(A)) {
      case "down-right":
        this.ctx.arc(o + s, w + s, s, Math.PI, Math.PI * 1.5), this.ctx.moveTo(o, Math.round(g + I / 2 - s)), this.ctx.lineTo(o, g), this.ctx.moveTo(Math.round(Q + E / 2 + s), w), this.ctx.lineTo(Q + E, w);
        break;
      case "down-left":
        this.ctx.arc(o - s, w + s, s, Math.PI * 1.5, Math.PI * 2), this.ctx.moveTo(o, Math.round(g + I / 2 - s)), this.ctx.lineTo(o, g), this.ctx.moveTo(Math.round(Q + E / 2 - s), w), this.ctx.lineTo(Q, w);
        break;
      case "up-left":
        this.ctx.arc(o - s, w - s, s, 0, Math.PI * 0.5), this.ctx.moveTo(o, Math.round(g + I / 2 + s)), this.ctx.lineTo(o, g + I), this.ctx.moveTo(Math.round(Q + E / 2 - s), w), this.ctx.lineTo(Q, w);
        break;
      case "up-right":
        this.ctx.arc(o + s, w - s, s, Math.PI * 0.5, Math.PI), this.ctx.moveTo(o, Math.round(g + I / 2 + s)), this.ctx.lineTo(o, g + I), this.ctx.moveTo(Math.round(Q + E / 2 + s), w), this.ctx.lineTo(Q + E, w);
        break;
    }
    this.ctx.stroke();
  }
   ...
   * @param fgColor The foreground color to use
   */
  drawDashedLine(A, Q, g, C) {
    const E = rA(A);
    if (!E)
      return;
    const I = this.metrics.width, D = this.metrics.height, o = Math.max(1, Math.round(D / 12)), w = E.weight === "heavy" ? o * 2 : o;
    this.ctx.fillStyle = C;
    const s = Math.round(Q + I / 2), t = Math.round(g + D / 2);
    if (E.direction === "horizontal") {
      const e = I / (E.dashes * 2 - 1), M = Math.round(t - w / 2);
      for (let a = 0; a < E.dashes; a++) {
        const h = Math.round(Q + a * e * 2);
        this.ctx.fillRect(h, M, Math.round(e), w);
      }
    } else {
      const e = D / (E.dashes * 2 - 1), M = Math.round(s - w / 2);
      for (let a = 0; a < E.dashes; a++) {
        const h = Math.round(g + a * e * 2);
        this.ctx.fillRect(M, h, w, Math.round(e));
      }
    }
  }
  /**
  ...
  renderCursor(A, Q) {
    ...
    switch (this.ctx.fillStyle = this.theme.cursor, this.cursorStyle) {
      case "block":
        this.ctx.fillRect(g, C, this.metrics.width, this.metrics.height);
        break;
      case "underline":
        const E = Math.max(2, Math.floor(this.metrics.height * 0.15));
        this.ctx.fillRect(
          g,
          C + this.metrics.height - E,
          this.metrics.width,
          E
        );
        break;
      case "bar":
        ...
        this.ctx.fillRect(g, C, I, this.metrics.height);
        break;
    }
  }
  // ==========================================================================
   ...
   */
  renderScrollbar(A, Q, g, C = 1) {
    const E = this.ctx, I = this.canvas.height / this.devicePixelRatio, D = this.canvas.width / this.devicePixelRatio, o = 8, w = D - o - 4, s = 4, t = I - s * 2;
    if (E.fillStyle = this.theme.background, E.fillRect(w - 2, 0, o + 6, I), C <= 0 || Q === 0)
      return;
    const e = Q + g, M = Math.max(20, g / e * t), a = A / Q, h = s + (t - M) * (1 - a);
    E.fillStyle = `rgba(128, 128, 128, ${0.1 * C})`, E.fillRect(w, s, o, t);
    const N = A > 0 ? 0.5 : 0.3;
    E.fillStyle = `rgba(128, 128, 128, ${N * C})`, E.fillRect(w, h, o, M);
  }
  getMetrics() {
    return { ...this.metrics };
  }
  ...
  isInSelection(A, Q) {
    ...
    if (!g)
      return !1;
    const { startCol: C, startRow: E, endCol: I, endRow: D } = g;
    return E === D ? Q === E && A >= C && A <= I : Q === E ? A >= C : Q === D ? A <= I : Q > E && Q < D;
  }
  /**
...
const S = class Y {
  ...
  getSelection() {
    if (!this.selectionStart || !this.selectionEnd)
      return "";
    let { col: A, absoluteRow: Q } = this.selectionStart, { col: g, absoluteRow: C } = this.selectionEnd;
    (Q > C || Q === C && A > g) && ([A, g] = [g, A], [Q, C] = [C, Q]);
    ...
    for (let D = Q; D <= C; D++) {
      ...
      if (!o)
        continue;
      let w = -1;
      const s = D === Q ? A : 0, t = D === C ? g : o.length - 1;
      let e = "";
      for (let M = s; M <= t; M++) {
        const a = o[M];
        if (a && a.codepoint !== 0) {
          ...
          if (a.grapheme_len > 0)
            if (D < E)
              ...
              const c = D - E;
              h = this.wasmTerm.getGraphemeString(c, M);
            }
          else
            h = String.fromCodePoint(a.codepoint);
          e += h, h.trim() && (w = e.length);
        } else
          e += " ";
      }
      w >= 0 ? e = e.substring(0, w) : e = "", I += e, D < C && (I += `
`);
    }
    return I;
  }
   ...
   */
  select(A, Q, g) {
    const C = this.wasmTerm.getDimensions();
    Q = Math.max(0, Math.min(Q, C.rows - 1)), A = Math.max(0, Math.min(A, C.cols - 1));
    let E = Q, I = A + g - 1;
    for (; I >= C.cols; )
      I -= C.cols, E++;
    E = Math.min(E, C.rows - 1);
    const D = this.getViewportY();
    this.selectionStart = { col: A, absoluteRow: D + Q }, this.selectionEnd = { col: I, absoluteRow: D + E }, this.requestRender(), this.selectionChangedEmitter.fire();
  }
  /**
  ...
  selectLines(A, Q) {
    ...
    A = Math.max(0, Math.min(A, g.rows - 1)), Q = Math.max(0, Math.min(Q, g.rows - 1)), A > Q && ([A, Q] = [Q, A]);
    const C = this.getViewportY();
    this.selectionStart = { col: 0, absoluteRow: C + A }, this.selectionEnd = { col: g.cols - 1, absoluteRow: C + Q }, this.requestRender(), this.selectionChangedEmitter.fire();
  }
  /**
  ...
  attachEventListeners() {
    ...
    A.addEventListener("mousedown", (Q) => {
      if (Q.button === 0) {
        ...
        const E = this.viewportRowToAbsolute(g.row);
        this.selectionStart = { col: g.col, absoluteRow: E }, this.selectionEnd = { col: g.col, absoluteRow: E }, this.isSelecting = !0;
      }
    }), A.addEventListener("mousemove", (Q) => {
      if (this.isSelecting) {
        this.markCurrentSelectionDirty();
        const g = this.pixelToCell(Q.offsetX, Q.offsetY), C = this.viewportRowToAbsolute(g.row);
        this.selectionEnd = { col: g.col, absoluteRow: C }, this.requestRender(), this.updateAutoScroll(Q.offsetY, A.clientHeight);
      }
      ...
      if (this.isSelecting) {
        const g = A.getBoundingClientRect(), C = Math.max(g.left, Math.min(Q.clientX, g.right)), E = Math.max(g.top, Math.min(Q.clientY, g.bottom)), I = C - g.left, D = E - g.top;
      ...
      const g = this.pixelToCell(Q.offsetX, Q.offsetY), C = this.getWordAtCell(g.col, g.row);
      if (C) {
        const E = this.viewportRowToAbsolute(g.row);
      ...
      if (this.renderer.getCanvas().getBoundingClientRect(), this.textarea.style.position = "fixed", this.textarea.style.left = `${Q.clientX}px`, this.textarea.style.top = `${Q.clientY}px`, this.textarea.style.width = "1px", this.textarea.style.height = "1px", this.textarea.style.zIndex = "1000", this.textarea.style.opacity = "0", this.textarea.style.pointerEvents = "auto", this.hasSelection()) {
        ...
        this.textarea.value = C, this.textarea.select(), this.textarea.setSelectionRange(0, C.length);
      } else
        this.textarea.value = "";
      this.textarea.focus(), setTimeout(() => {
        const C = () => {
          this.textarea.style.pointerEvents = "none", this.textarea.style.zIndex = "-10", this.textarea.style.width = "0", this.textarea.style.height = "0", this.textarea.style.left = "0", this.textarea.style.top = "0", this.textarea.value = "", document.removeEventListener("click", C), document.removeEventListener("contextmenu", C), this.textarea.removeEventListener("blur", C);
        };
        document.addEventListener("click", C, { once: !0 }), document.addEventListener("contextmenu", C, { once: !0 }), this.textarea.addEventListener("blur", C, { once: !0 });
      }, 10);
      ...
      if (this.isSelecting || this.mouseDownTarget && A.contains(this.mouseDownTarget))
        return;
      const C = Q.target;
      A.contains(C) || this.hasSelection() && this.clearSelection();
  ...
  startAutoScroll(A) {
    this.autoScrollInterval !== null && this.autoScrollDirection === A || (this.stopAutoScroll(), this.autoScrollDirection = A, this.autoScrollInterval = setInterval(() => {
      ...
      const Q = Y.AUTO_SCROLL_SPEED * this.autoScrollDirection;
      if (this.terminal.scrollLines(Q), this.selectionEnd) {
        const g = this.wasmTerm.getDimensions();
        if (this.autoScrollDirection < 0) {
          const C = this.viewportRowToAbsolute(0);
          C < this.selectionEnd.absoluteRow && (this.selectionEnd = { col: 0, absoluteRow: C });
        } else {
          const C = this.viewportRowToAbsolute(g.rows - 1);
          C > this.selectionEnd.absoluteRow && (this.selectionEnd = { col: g.cols - 1, absoluteRow: C });
        }
      }
      this.requestRender();
   ...
   */
  pixelToCell(A, Q) {
    const g = this.renderer.getMetrics(), C = Math.floor(A / g.width), E = Math.floor(Q / g.height);
    return {
      col: Math.max(0, Math.min(C, this.terminal.cols - 1)),
      row: Math.max(0, Math.min(E, this.terminal.rows - 1))
    };
  }
  /**
  ...
  normalizeSelection() {
    ...
    let { col: A, absoluteRow: Q } = this.selectionStart, { col: g, absoluteRow: C } = this.selectionEnd;
    (Q > C || Q === C && A > g) && ([A, g] = [g, A], [Q, C] = [C, Q]);
    let E = this.absoluteRowToViewport(Q), I = this.absoluteRowToViewport(C);
    const D = this.wasmTerm.getDimensions(), o = D.rows - 1;
    return I < 0 || E > o ? null : (E < 0 && (E = 0, A = 0), I > o && (I = o, g = D.cols - 1), { startCol: A, startRow: E, endCol: g, endRow: I });
  }
  /**
   * Get word boundaries at a cell position
   */
  getWordAtCell(A, Q) {
    const g = this.wasmTerm.getLine(Q);
    if (!g)
      return null;
    const C = (D) => {
      if (!D || D.codepoint === 0)
    ...
    for (; E > 0 && C(g[E - 1]); )
      E--;
    let I = A;
    for (; I < g.length - 1 && C(g[I + 1]); )
      I++;
    return { startCol: E, endCol: I };
  }
  /**
  ...
  copyToClipboard(A) {
    if (navigator.clipboard && typeof ClipboardItem < "u")
      try {
        ...
        navigator.clipboard.write([g]).catch((C) => {
          console.warn("ClipboardItem write failed, trying writeText:", C), this.copyWithWriteText(A);
        });
        return;
      } catch {
      }
  ...
  copyWithExecCommand(A) {
    ...
    try {
      ...
      Q && Q.focus(), C || console.warn("execCommand copy failed");
    } catch (g) {
...
class LA {
  ...
  constructor(A = {}) {
    this.unicode = {
      ...
      if (!this.wasmTerm || this.scrollAnimationStartTime === void 0)
        return;
      const g = this.options.smoothScrollDuration ?? 100, C = this.targetViewportY - this.viewportY;
      if (Math.abs(C) < 0.01) {
        ...
        if (this.linkDetector) {
          ...
          this.processMouseMove(g), this.mouseMoveThrottleTimeout = window.setTimeout(() => {
            if (this.mouseMoveThrottleTimeout = void 0, this.pendingMouseMove) {
              const C = this.pendingMouseMove;
              this.pendingMouseMove = void 0, this.processMouseMove(C);
            }
          }, 16);
        }
      }
      ...
      var g, C;
      this.renderer && this.wasmTerm && ((this.renderer.hoveredHyperlinkId || 0) > 0 && this.renderer.setHoveredHyperlinkId(0), this.renderer.setHoveredLinkRange(null)), this.currentHoveredLink && ((C = (g = this.currentHoveredLink).hover) == null || C.call(g, !1), this.currentHoveredLink = void 0, this.element && (this.element.style.cursor = "text"));
    }, this.handleClick = async (g) => {
      if (!this.canvas || !this.renderer || !this.linkDetector || !this.wasmTerm)
        return;
      const C = this.canvas.getBoundingClientRect(), E = Math.floor((g.clientX - C.left) / this.renderer.charWidth), D = Math.floor((g.clientY - C.top) / this.renderer.charHeight), o = this.wasmTerm.getScrollbackLength();
      let w;
      ...
      if (t > 0)
        ...
        w = o + D;
      const e = await this.linkDetector.getLinkAt(E, w);
      e && (e.activate(g), (g.ctrlKey || g.metaKey) && g.preventDefault());
    }, this.handleWheel = (g) => {
      var E, I, D;
      if (g.preventDefault(), g.stopPropagation(), this.customWheelEventHandler && this.customWheelEventHandler(g))
        return;
      if (((E = this.wasmTerm) == null ? void 0 : E.isAlternateScreen()) ?? !1) {
      ...
      if (C === 0)
        return;
      const E = this.canvas.getBoundingClientRect(), I = g.clientX - E.left, D = g.clientY - E.top, o = E.width, w = E.height, s = 8, t = o - s - 4, e = 4;
      if (I >= t && I <= t + s) {
        g.preventDefault(), g.stopPropagation(), g.stopImmediatePropagation();
        const M = w - e * 2, a = this.rows, h = C + a, c = Math.max(20, a / h * M), N = this.viewportY / C, r = e + (M - c) * (1 - N);
        if (D >= r && D <= r + c)
          ...
          const F = 1 - (D - e) / M, n = Math.round(F * C);
          this.scrollToLine(Math.max(0, Math.min(C, n)));
        }
      }
    }, this.handleMouseUp = () => {
    ...
    this.options = new Proxy(Q, {
      set: (g, C, E) => {
        const I = g[C];
        return g[C] = E, this.isOpen && this.handleOptionChange(C, E, I), !0;
      }
  ...
  parseColorToHex(A) {
    ...
    if (Q) {
      const g = Number.parseInt(Q[1], 10), C = Number.parseInt(Q[2], 10), E = Number.parseInt(Q[3], 10);
      return g << 16 | C << 8 | E;
    }
    return 0;
  }
  ...
  open(A) {
    ...
    try {
      ...
      const C = this.canvas, E = this.renderer, I = this.wasmTerm, D = {
      ...
      this.inputHandler = new X(
        this.ghostty,
        A,
        (o) => {
          this.options.disableStdin || this.dataEmitter.fire(o);
        },
        () => {
...
`, Q);
  ...
  select(A, Q, g) {
    var C;
    (C = this.selectionManager) == null || C.select(A, Q, g);
  }
  /**
   * Select entire lines from start to end
   */
  ...
  scrollLines(A) {
    ...
    const Q = this.getScrollbackLength(), C = Math.max(0, Math.min(Q, this.viewportY - A));
    C !== this.viewportY && (this.viewportY = C, this.scrollEmitter.fire(this.viewportY), Q > 0 && this.showScrollbar());
  }
  /**
  ...
  smoothScrollTo(A) {
    if (!this.wasmTerm)
      return;
    const Q = this.getScrollbackLength(), C = Math.max(0, Math.min(Q, A));
    if ((this.options.smoothScrollDuration ?? 100) === 0) {
      this.viewportY = C, this.targetViewportY = C, this.scrollEmitter.fire(Math.floor(this.viewportY)), Q > 0 && this.showScrollbar();
      return;
    }
    this.targetViewportY = C, !this.scrollAnimationFrame && (this.scrollAnimationStartTime = Date.now(), this.scrollAnimationStartY = this.viewportY, this.animateScroll());
  }
  // ==========================================================================
  // Lifecycle
  // ==========================================================================
  ...
  processMouseMove(A) {
    if (!this.canvas || !this.renderer || !this.linkDetector || !this.wasmTerm)
      return;
    const Q = this.canvas.getBoundingClientRect(), g = Math.floor((A.clientX - Q.left) / this.renderer.charWidth), E = Math.floor((A.clientY - Q.top) / this.renderer.charHeight);
    let I = 0, D = null;
    ...
    if (w > 0) {
      ...
      if (E < w) {
        const c = h - w + E;
        D = this.wasmTerm.getScrollbackLine(c);
      } else {
        const c = E - w;
    ...
    if (a > 0)
      ...
      e = t + E;
    this.linkDetector.getLinkAt(g, e).then((h) => {
      var c, N, r, k;
      if (h !== this.currentHoveredLink && ((N = (c = this.currentHoveredLink) == null ? void 0 : c.hover) == null || N.call(c, !1), this.currentHoveredLink = h, (r = h == null ? void 0 : h.hover) == null || r.call(h, !0), this.element && (this.element.style.cursor = h ? "pointer" : "text"), this.renderer))
        if (h) {
          const F = ((k = this.wasmTerm) == null ? void 0 : k.getScrollbackLength()) || 0, n = this.getViewportY(), L = Math.max(0, Math.floor(n)), d = h.range.start.y - F + L, T = h.range.end.y - F + L;
  ...
  processScrollbarDrag(A) {
    ...
    if (Q === 0)
      return;
    const g = this.canvas.getBoundingClientRect(), E = A.clientY - g.top - this.scrollbarDragStart, o = g.height - 4 * 2, w = this.rows, s = Q + w, t = Math.max(20, w / s * o), e = -E / (o - t), M = Math.round(e * Q), a = this.scrollbarDragStartViewportY + M;
  ...
  fadeInScrollbar() {
    const A = Date.now(), Q = () => {
      ...
      this.scrollbarOpacity = C, this.renderer && this.wasmTerm && this.renderer.render(this.wasmTerm, !1, this.viewportY, this, this.scrollbarOpacity), C < 1 && requestAnimationFrame(Q);
    };
    Q();
  }
  /**
  ...
  fadeOutScrollbar() {
    const A = Date.now(), Q = this.scrollbarOpacity, g = () => {
      const C = Date.now() - A, E = Math.min(C / this.SCROLLBAR_FADE_DURATION_MS, 1);
      this.scrollbarOpacity = Q * (1 - E), this.renderer && this.wasmTerm && this.renderer.render(this.wasmTerm, !1, this.viewportY, this, this.scrollbarOpacity), E < 1 ? requestAnimationFrame(g) : (this.scrollbarVisible = !1, this.scrollbarOpacity = 0, this.renderer && this.wasmTerm && this.renderer.render(this.wasmTerm, !1, this.viewportY, this, 0));
    };
    g();
  }
  /**
  ...
  checkForTitleChange(A) {
    ...
    for (; (g = Q.exec(A)) !== null; ) {
      const C = g[1], E = g[2];
      (C === "0" || C === "2") && E !== this.currentTitle && (this.currentTitle = E, this.titleChangeEmitter.fire(E));
    }
  }
  // ============================================================================
  // Terminal Modes
...
class YA {
  ...
  fit() {
    ...
    if (!A || !this._terminal)
      return;
    const Q = this._terminal, g = Q.cols, C = Q.rows;
    if (!(A.cols === this._lastCols && A.rows === this._lastRows || A.cols === g && A.rows === C)) {
  ...
  proposeDimensions() {
    var c;
    if (!((c = this._terminal) != null && c.element))
      return;
    ...
    if (!g || g.width === 0 || g.height === 0)
      return;
    const C = this._terminal.element;
    if (typeof C.clientWidth > "u")
      return;
    const E = window.getComputedStyle(C), I = Number.parseInt(E.getPropertyValue("padding-top")) || 0, D = Number.parseInt(E.getPropertyValue("padding-bottom")) || 0, o = Number.parseInt(E.getPropertyValue("padding-left")) || 0, w = Number.parseInt(E.getPropertyValue("padding-right")) || 0, s = C.clientWidth, t = C.clientHeight;
    if (s === 0 || t === 0)
      return;
    const e = s - o - w - KA, M = t - I - D, a = Math.max(UA, Math.floor(e / g.width)), h = Math.max(yA, Math.floor(M / g.height));
...
export {
  ...
  _ as LinkDetector,
  $ as OSC8LinkProvider,
  JA as SelectionManager,
  LA as Terminal,
  AA as UrlRegexProvider,
  qA as getGhostty,
  RA as init
};
